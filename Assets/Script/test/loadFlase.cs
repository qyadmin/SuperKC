...
namespace Load {
	...
	public sealed class falselist {
		...
		public static string Creat(string str,string key)
		{
			string stradd = string.Empty;
			//是否是调试模式
			if (Static.Instance.IsDebug) {
				Debug.Log ("当前为调试模式，不可瞎想");
			} else {
				Byte[] encrypt_data = Encrypt (str, key);
				//String decrypt_data = Xxtea.XXTEA.DecryptToString(encrypt_data, key);

				foreach (byte child in encrypt_data)
					stradd += child.ToString("x2");			
			}
		//	Debug.Log (stradd);
			return stradd;
		}
