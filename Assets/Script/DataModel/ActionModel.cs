...
{
    public string Tag;
    public MessageEvent<JsonData> Config = new MessageEvent<JsonData>();

    public ConfigTag()
    {
        ModelManager.GetModelManager.ServerBody.Addlistener(GetMessage);
        //绑定主体事件
    }

    public void Addself(string Tag,Dictionary<string, MessageEvent<JsonData>> ConfigBody)
    {
        this.Tag = Tag;
...
public class EventConfig
{
    public EventConfig(string name,string Tag,string id)
    {
        ...
        this.id = id;
        //绑定层级事件
        MessageEvent<JsonData> obj = ModelManager.GetModelManager.GetBody(Tag);
        if (obj != null)
...
public class GetCL
{
    ...
    public static int Getlvlint(string lvl)
    {
        ...
        {
            lvl = lvl == "0" ? "2" : lvl;
            int nub = System.Convert.ToInt32(Mathf.Floor((int.Parse(lvl)-1) / 5));
            nub = nub > 3 ? 3 : nub;
            return ((nub + 1) * 5);
        }
        return trynub;
